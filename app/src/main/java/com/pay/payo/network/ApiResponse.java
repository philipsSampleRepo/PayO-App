...
public class ApiResponse<T> {
    ...
    public ApiResponse<T> create(Throwable error) {
        ...
        return new ApiErrorResponse<>(error.getMessage().equals("")
                ? Constants.NETWORK_ERROR : error.getMessage());
    }
    ...
    public ApiResponse<T> create(Response<T> response) {
        ...
        if (response.isSuccessful()) {
            ...
            if (body instanceof NetworksResponseModel) {
                if (!((NetworksResponseModel) body).getReturnCode().getName().equals("OK")) {
                    String errorMsg = Constants.RESPONSE_ERROR;
            ...
            try {
                if (response.code() == 404) {
                    errorMsg = Constants.ERROR_404;
                } else if (response.code() == 500) {
                    errorMsg = Constants.ERROR_500;
                } else {
